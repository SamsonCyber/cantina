"""
Cantina plugin: mssql_enum (replaces built-in mssql recon).
...
PLUGIN = {
    ...
    "description": "MSSQL info / NTLM enum",
    "priority": 50,
...
def run(ctx):
    ...
    arts = []
    notes = []
    scripts = "ms-sql-info,ms-sql-empty-password,ms-sql-ntlm-info"
    extra = ""
    ofile = pdir / "nmap_mssql.txt"
    content = ""
    if scripts:
        ...
        if content:
            ...
            if "vulnerable" in low:
                finding(ctx, "CRITICAL", "MSSQL", "MSSQL vulnerability signal in nmap output")
                notes.append("vulnerable_signal")
            if "empty-password" in low or "empty password" in low:
                finding(ctx, "CRITICAL", "MSSQL", "Empty password signal")
                notes.append("empty_password")
            if "dns_computer_name" in low or "dns_domain_name" in low:
                finding(ctx, "INFO", "MSSQL", "NTLM/DNS name leak in nmap output")
    ...
    return write_summary(ctx, "mssql_enum", notes, arts)

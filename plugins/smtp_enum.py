"""
Cantina plugin: smtp_enum (replaces built-in smtp recon).
...
PLUGIN = {
    ...
    "description": "SMTP commands / open-relay enum",
    "priority": 50,
    "legal": "enumeration-only; OSCP-safe; no exploit/spray auto-run",
}

_NAMES = set(['smtp'])
_PORTS = set([25, 465, 587])
...
def run(ctx):
    ...
    if scripts:
        ...
        if content:
            ...
            if "vulnerable" in low:
                finding(ctx, "CRITICAL", "SMTP", "SMTP vulnerability signal in nmap output")
                notes.append("vulnerable_signal")
            if "empty-password" in low or "empty password" in low:
                finding(ctx, "CRITICAL", "SMTP", "Empty password signal")
                notes.append("empty_password")
            if "dns_computer_name" in low or "dns_domain_name" in low:
                finding(ctx, "INFO", "SMTP", "NTLM/DNS name leak in nmap output")
    ...
    return write_summary(ctx, "smtp_enum", notes, arts)

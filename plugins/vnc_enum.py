"""
Cantina plugin: vnc_enum (replaces built-in vnc recon).
...
PLUGIN = {
    ...
    "description": "VNC info (no vnc-brute)",
    "priority": 50,
    "legal": "enumeration-only; OSCP-safe; no exploit/spray auto-run",
}

_NAMES = set(['vnc'])
_PORTS = set([5900, 5901, 5902])
...
def run(ctx):
    ...
    if scripts:
        ...
        if content:
            ...
            if "vulnerable" in low:
                finding(ctx, "CRITICAL", "VNC", "VNC vulnerability signal in nmap output")
                notes.append("vulnerable_signal")
            if "empty-password" in low or "empty password" in low:
                finding(ctx, "CRITICAL", "VNC", "Empty password signal")
                notes.append("empty_password")
            if "dns_computer_name" in low or "dns_domain_name" in low:
                finding(ctx, "INFO", "VNC", "NTLM/DNS name leak in nmap output")
    ...
    return write_summary(ctx, "vnc_enum", notes, arts)

# Cantina lab target (CT 206)

Homelab multi-service OSCP-style enum surface for scoring cantina deep mode.

| Field | Value |
|-------|--------|
| Proxmox | pve1 (`192.168.1.11`) |
| CT | 206 `cantina-lab` |
| IP | `192.168.1.206` |
...
sudo python3 ~/cantina-tools/cantina.py 192.168.1.206 -t deep --background \
  -o ~/cantina-live/192.168.1.206 -j --rate 4

# poll progress (does not block short tools)
python3 ~/cantina-tools/cantina.py --status -o ~/cantina-live/192.168.1.206

# score vs ground truth (any time; re-run after complete for final)
python3 ~/cantina-tools/cantina_live_bench.py \
  -o ~/cantina-live/192.168.1.206 \
